...
export const useReaderGestures = ({
  ...
  const panGesture = Gesture.Pan()
    // Sadece sağ %15'lik şeritte aktifleşsin — aksi halde tüm ekrandaki
    // sayfa çevirme / liste kaydırma dokunuşlarını WebView'den çalar.
    .hitSlop({ right: 0, width: width * 0.15 })
    .onBegin(() => {
      // no-op
    })
    .onUpdate((e) => {
